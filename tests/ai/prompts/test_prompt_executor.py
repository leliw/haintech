...
@pytest.mark.asyncio
async def test_execute_typed_async(prompt_executor: PromptExecutor, mocker_ai_model: MockerAIModel):
    mocker_ai_model.add(
        message_containing="{'properties': {'content': {'title': 'Content', 'type': 'string'}}, 'required': ['content'], 'title': 'Output', 'type': 'object'}",
        response='{"content": "Welcome to the fascinating world of English verbs! Today, we\'re diving into one of the most fundamental and frequently used verbs: \'to be\'." }',
    )
    # When:
    ret = await prompt_executor.execute_typed_async(
        "test_output", InfoPageCreate, title="Verb to be - Introduction", language="en", level="A1"
    )
    # Then: A InfoPageCreate objects is returned after conversion
    assert isinstance(ret, InfoPageCreate)
...
@pytest.mark.asyncio
async def test_execute_typed_list_async(prompt_executor: PromptExecutor, mocker_ai_model: MockerAIModel):
    mocker_ai_model.add_calls(
        [
            {
                "system_prompt": "",
    ...
    # When:
    ret = await prompt_executor.execute_typed_list_async("book_list", Book)
    # Then: A InfoPageCreate objects is returned after conversion
    assert all(isinstance(book, Book) for book in ret)


@pytest.mark.asyncio
async def test_execute_list_async(prompt_executor: PromptExecutor, mocker_ai_model: MockerAIModel):
    mocker_ai_model.add_calls(
        [
            {
                "system_prompt": "",
    ...
    # When:
    ret = await prompt_executor.execute_list_async("book_years", int)
    # Then: A InfoPageCreate objects is returned after conversion
    assert all(isinstance(year, int) for year in ret)


@pytest.mark.asyncio
async def test_execute_with_blob_async(prompt_executor: PromptExecutor, mocker_ai_model: MockerAIModel):
    mocker_ai_model.add_calls(
        [
            {
                "system_prompt": "",
    ...
    blob = factory.download_blob(blob_location)
    # When:
    ret = await prompt_executor.execute_async("picture_descriptor", blobs=[blob])
    # Then: A InfoPageCreate objects is returned after conversion
    assert "dog" in ret.lower()
